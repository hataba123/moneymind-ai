  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_1.2fr]">
      <section className="money-card rounded-lg p-5">
        <h2 className="font-semibold text-slate-900">Hỏi MoneyMind AI</h2>
        <div className="mt-4 flex flex-wrap gap-2">
          {prompts.map((prompt) => (
            <button key={prompt} type="button" onClick={() => setQuestion(prompt)} className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700 hover:bg-blue-100">
              {prompt}
            </button>
          ))}
        </div>
        <form onSubmit={ask} className="mt-5 space-y-3">
          ...
          <button disabled={sending} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-md bg-blue-700 font-semibold text-white hover:bg-blue-800 disabled:opacity-60">
            <Send size={17} /> {sending ? "Đang phân tích..." : "Gửi câu hỏi"}
          </button>
        </form>
      </section>
      <section className="money-card rounded-lg p-5">
        <h2 className="font-semibold text-slate-900">Lịch sử tư vấn</h2>
        <div className="mt-4 space-y-4">
          {loading ? <p className="text-sm text-slate-500">Đang tải...</p> : logs.length ? logs.map((log) => (
            <article key={log.id} className="rounded-md border border-slate-100 bg-white p-4">
              <p className="font-semibold text-slate-900">{log.question}</p>
